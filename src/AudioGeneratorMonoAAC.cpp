...
AudioGeneratorMonoAAC::AudioGeneratorMonoAAC()
{
  ...
  outSample = (int16_t*)malloc(1024 * sizeof(uint16_t));
  if (!buff || !outSample) {
    audioLogger->printf_P(PSTR("ERROR: Out of memory in AAC\n"));
    Serial.flush();
  }
  memset(buff, 0, buffLen);
  memset(outSample, 0, 2048);
...
AudioGeneratorMonoAAC::AudioGeneratorMonoAAC(void *preallocateData, int preallocateSz)
{
...
bool AudioGeneratorMonoAAC::stop()
{
  running = false;
  output->stop();
  return file->close();
...
bool AudioGeneratorMonoAAC::isRunning()
{
  return running;
}

bool AudioGeneratorMonoAAC::FillBufferWithValidFrame()
{
...
bool AudioGeneratorMonoAAC::loop()
{
...
bool AudioGeneratorMonoAAC::begin(AudioFileSource *source, AudioOutput *output)
{
  ...
  memset(buff, 0, buffLen);
  memset(outSample, 0, 1024 * sizeof(int16_t));
